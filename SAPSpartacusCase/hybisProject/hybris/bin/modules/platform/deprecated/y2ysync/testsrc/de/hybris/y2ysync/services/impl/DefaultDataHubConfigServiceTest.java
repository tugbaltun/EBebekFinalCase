...
{
	private static final String REGEXP_TRIM_WHITECHARS = "[\\r\\n]+[\\t\\s]+";
	@Resource
	...
	public void shouldGenerateDataHubModelForStreamConfiguration() throws Exception
	{
		...
		assertThat(modelDefinitions.replaceAll(REGEXP_TRIM_WHITECHARS, StringUtils.EMPTY)).isXmlEqualTo(
				getExpectedXmlFromResource("dataHubModelFromStreamConfig.xml").replaceFirst("<!--[\\s\\S]*?-->",
						StringUtils.EMPTY));
	}
	...
	public void shouldGenerateDataHubModelForStreamConfigurationContainer() throws Exception
	{
		...
		// then
		assertThat(modelDefinitions).isNotNull();
		final String expectedXml = getExpectedXmlFromResource("dataHubModelFromStreamConfigContainer.xml").replaceFirst(
				"<!--[\\s\\S]*?-->", StringUtils.EMPTY);
		assertThat(modelDefinitions.replaceAll(REGEXP_TRIM_WHITECHARS, StringUtils.EMPTY)).isXmlEqualTo(expectedXml);
	}
	...
	public void shouldGenerateDataHubModelForStreamConfigurationWithUntypedColumnDefintion() throws Exception
	{
		...
		// then
		assertThat(modelDefinitions).isNotNull();
		assertThat(modelDefinitions.replaceAll(REGEXP_TRIM_WHITECHARS, StringUtils.EMPTY))
				.isXmlEqualTo(getExpectedXmlFromResource(
						"dataHubModelForStreamConfigurationWithUntypedColumnDefintion.xml").replaceFirst("<!--[\\s\\S]*?-->",
						StringUtils.EMPTY));
	}
	...
	private String getExpectedXmlFromResource(final String fileName)
	{
		...
		try
		{
			return IOUtils.toString(resource, "UTF-8").replaceAll(REGEXP_TRIM_WHITECHARS, StringUtils.EMPTY);
		}
