...
public class YDocumentCartRepositoryTest extends ServicelayerBaseTest
{
	...
	public void testCartVersion()
	{
		...
		//then
		assertThat(result.<Long>get(version)).isEqualTo(1L);
	}
	...
	public void testCartWithCartEntryVersionAfterUpdate()
	{
		...
		//then
		assertThat(cartState.<Long>get(version)).isLessThan(cartEntryState.<Long>get(version));
		assertThat(cartState).isInstanceOf(Entity.class);
		assertThat(cartEntryState).isInstanceOf(Entity.class);

		final Document cartEntryDocument = ((Entity) cartEntryState).getDocument();
		final Document cartDocument = ((Entity) cartState).getDocument();

		assertThat(cartEntryDocument).extracting(Document::getRootId, Document::getVersion)
		                             .containsExactly(cartDocument.getRootId(), cartDocument.getVersion());
	...
	public void testCartVersionIsUnchangedAfterUpdateOfCartEntry()
	{
		...
		//then
		assertThat(cartStateAfter.<Long>get(version)).isEqualTo(cartStateBefore.<Long>get(version));
	}
